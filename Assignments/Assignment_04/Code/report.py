...
if __name__ == '__main__':
    ...
    parser.add_argument('--outputDir', type=str, default='../report/', help='Output directory')
    ...
    for index in range(len(offsets)):
        ...
        if not isMix:
            # Display the result
            fig, ax = plt.subplots(1, 5, figsize=(12, 4))  # Adjusted figure size for better visibility
            naiveOutput = source * mask + target * (1 - mask)

            # Iterate over axes to display images
            images = [source, mask, target, naiveOutput, poissonOutput]
            titles = ['Source', 'Mask', 'Target', 'Naive Blending', 'Poisson Blending']

            for i, (img, title) in enumerate(zip(images, titles)):
                ax[i].imshow(img)
                ax[i].set_title(title)
                ax[i].axis('off')  # Remove axis ticks

            # plt.suptitle(f'Poisson Blending - Example {index+1}', fontsize=14)  # Adjusted font size and added vertical spacing
            plt.subplots_adjust(wspace=0.3, hspace=0.3)  # Adjust spacing between subplots
            plt.tight_layout()
            plt.savefig("{}poisson_{}.jpg".format(outputDir, str(index+1).zfill(2)))
        else:
            
            images = [source, mask, target,  PoissonBlend(source, mask, target, isMix=False), poissonOutput] 
            titles = ['Source', 'Mask', 'Target', 'Poisson Blending', 'Mixing Gradient']
            ...
            plt.savefig("{}poisson_{}_Mixing.jpg".format(outputDir, str(index+1).zfill(2)))